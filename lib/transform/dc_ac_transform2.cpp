...
////////////////////////////////////////////////////////////////////////////////
auto DCACTransform2::process(const std::vector<std::int32_t>& coeffs) -> Ret {
    auto [dc, ac] = _splitACDC(coeffs);
    auto acReordered = _acZigZagReorder(ac);
    auto [dcMoved, dcOffset, dcRng] = _rangeMove(dc);
    auto [acMoved, acOffset, acRng] = _rangeMove(ac);
    auto [acProcessed, acLengthes, acLengthesRng] = _acZip(acMoved, acRng, -acOffset);
    return {
        dcMoved,
        dcOffset,
        dcRng,
...
////////////////////////////////////////////////////////////////////////////////
std::vector<std::int32_t> DCACTransform2::processBack(
...
////////////////////////////////////////////////////////////////////////////////
auto DCACTransform2::_splitACDC(
...
////////////////////////////////////////////////////////////////////////////////
auto DCACTransform2::_rangeMove(
...
DCACTransform2::_acZigZagReorder(const std::vector<std::int32_t>& coeffs) {
...
////////////////////////////////////////////////////////////////////////////////
auto DCACTransform2::_acZip(
        const std::vector<std::uint32_t>& coeffs,
        std::uint32_t maxOrd,
        std::uint32_t movedZero) -> _ACZipResult {
    _ACZipResult ret;
    ret.lengthesRng = 1;
    for (auto iter = coeffs.begin(); iter < coeffs.end(); iter += 63) {
        std::uint32_t currZerosCnt = 0;
        bool foundNonZero = false;
        for (auto inBlockIter = iter; inBlockIter < iter + 63; ++inBlockIter) {
            if (*inBlockIter == movedZero) {
                ++currZerosCnt;
            } else {
                if (currZerosCnt != 0) {
                    ret.lengthesRng = std::max(ret.lengthesRng, currZerosCnt + 1);
                    ret.acZipped.push_back(movedZero);
                    ret.lengthes.push_back(currZerosCnt);    
                }
                ret.acZipped.push_back(*inBlockIter);
                currZerosCnt = 0;
                foundNonZero = true;
            }
        }
        if (!foundNonZero) {
            ret.lengthes.push_back(0);
        }
    }
    return ret;
}
...
////////////////////////////////////////////////////////////////////////////////
std::vector<std::int32_t> DCACTransform2::_joinACDC(
...
////////////////////////////////////////////////////////////////////////////////
std::vector<std::int32_t> DCACTransform2::_rangeMoveBack(
...
////////////////////////////////////////////////////////////////////////////////
std::vector<std::int32_t>
DCACTransform2::_acZigZagReorderBack(const std::vector<std::int32_t>& coeffs) {
    std::vector<std::int32_t> ret;
    for (auto iter = coeffs.begin(); iter < coeffs.end(); iter += 63) {
        for (std::size_t i = 0; i < 63; ++i) {
...
////////////////////////////////////////////////////////////////////////////////
std::vector<std::int32_t> DCACTransform2::_acTailsUnzip(
